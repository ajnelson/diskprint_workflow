    ...
    with open(args.output_file, "w", newline='\r\n') as output_fh:
        ...
        for (event, obj) in Objects.iterparse(args.input_dfxml):
            ...
            if obj.filesize > 0 and obj.data_brs:
                if len(obj.data_brs) > 0 and obj.data_brs[0].type == "resident":
                    _logger.warning("Due to an unresolved issue in Fiwalk and TSK, files with resident byte runs are skipped for now.")
                    continue
                ...
                try:
                    ...
                    if obj.filesize != read_bytes:
                        ...
                        any_error = True
                    if any_error:
                        _logger.error("Content mismatch between what Fiwalk computed and what this script could extract (id=%r).  Skipping." % obj.id)
