...
"$PYTHON3" "${script_dir}/fetch_node_data.py" --debug --config="$DIFFER_CONFIG" disk "$node_id" > "${dwf_output_dir}/disk_image_paths.txt"

while read disk_image_path; do
...
done < "${dwf_output_dir}/disk_image_paths.txt"
